  ...
  return (
    <div className={`${className || "w-full"}`}>
      <Ariakit.SelectProvider value={value} setValue={handleChange}>
        ...
        <Ariakit.SelectPopover className="mt-2 rounded shadow-lg bg-white dark:bg-surface/90 border border-brand/20 py-2 z-50 animate-fade-in max-h-60 overflow-y-auto">
          <Ariakit.SelectList>
            {items.map((item) => (
              <Ariakit.SelectItem
                key={item.value}
